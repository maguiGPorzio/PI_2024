...
/*
Escribir una función que reciba una matriz de números enteros y cuya cantidad de filas está dada por la constante 
simbólica FILS y la cantidad de columnas por la constante simbólica COLS.
...
si no es ascendente ni descendente. Se dice que una matriz es ascendente si recorriendola en forma 
ordenada por filas cada elemento es mayor o igual al anterior, En forma análoga se define una matriz descendente.
...
int 
ascendenteODescendente(int (m[])[COLS]){
	int ascendente=1,descendente=1,anterior=m[0][0];
	for (int i=0 ; i<FILS && (ascendente || descendente); i++){
		for (int j=0 ; j<COLS ; j++){
			if (m[i][j]<anterior){
				ascendente=0;
			}
			else if (m[i][j]>anterior){
				descendente=0;
			}
			anterior=m[i][j];
		}
