/*
Escribir una función secuenciaAsc que reciba un vector de enteros (sin orden) y su dimensión y 
almacene en dos parámetros de salida:
comienzo: Dónde comienza la secuencia de números ordenados en forma ascendente (cada elemento 
debe ser mayor al anterior) de mayor longitud
longitud: Longitud de esa secuencia

En caso de haber más de una secuencia con la mayor longitud debe almacenar en comienzo donde 
comienza la primera de ellas.
...
void 
secuenciaAsc(const int vec[], int dim, int *comienzo, int *longitud){
    *comienzo=0;
    *longitud=0;
    if (dim){
        *longitud=1;
        int auxCom=0, auxLon=1;
        for (int i=1 ; i<dim ; i++){
            if (vec[i]<=vec[i-1]){
                auxCom=i;
                auxLon=1;
