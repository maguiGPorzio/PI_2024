...
sumMatch(const int v[]){
    if (*v==-1){ //si tiene un unico elemento (el -1, la marca de fin)
        return 0;
    }

    if (*(v+1)==-1){ //cuando es el ultimo elemento antes de la marca de fin quiero que lo devuelva, caso base
        return *(v);
    }

    int proximo=sumMatch(v+1);

    if (proximo==0){ //cuando la resta dio cero quiero que se "inicialize" de nuevo 
        return (*v);
    }
